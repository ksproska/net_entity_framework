...
//                Console.WriteLine($"{elem.Number} -> {elem.Word}");
        ...
        static List<KeyValuePair<string, int>> TopicsCounter(List<StudentWithTopics> students)
        {
            ...
            foreach(StudentWithTopics student in students)
            {
                List<string> topics = student.Topics;
                foreach(string topic in topics)
                {
                    if(!dictionaryTopics.ContainsKey(topic))
                    {
                        dictionaryTopics.Add(topic, 0);
                    }
                    dictionaryTopics[topic] += 1;
                }
            }
            var sortedDict = from entry in dictionaryTopics orderby entry.Value descending select entry;
        ...
        static void Main()
        {
            //Zad1();
            Zad2();
            //studentsWithTopics.ForEach(Console.WriteLine);
            //Console.WriteLine("--------------");
